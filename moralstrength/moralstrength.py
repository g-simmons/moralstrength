...
def word_moral_annotations(word, normalized=False):
    ...
    return {
        moral: lexicon_use.moral_value(word=word, moral=moral, normalized=normalized)
...
def string_moral_value(text, moral, model="unigram+freq"):
    ...
    if model not in models:
        raise ValueError(
            'Invalid model "{}" specified. Valid models are: {}'.format(model, models)
        )
    if moral not in moral_options_predictions:
        raise ValueError(
            'Invalid moral trait "{}" specified. Valid traits are: {}'.format(
                moral, moral_options_predictions
            )
        )
...
def string_moral_values(text, model="unigram+freq"):
    ...
    if model not in models:
        raise ValueError(
            'Invalid model "{}" specified. Valid models are: {}'.format(moral, models)
        )
    return {
        moral: estimate(text=text, moral=moral, model=model)[0]
        for moral in moral_options_predictions
    }
